<script lang="ts" context="module">
	...
	type LightProp = { [K in keyof typeof lights]: [K, (typeof lights)[K]] }[keyof typeof lights];

	function updateMaterial(props: MaterialProp[]) {
		for (const [key, val] of props) {
			(material as any)[key] = val;
		}
	}

	function updateLights(props: LightProp[]) {
		for (const [key, val] of props) {
			(lights as any)[key] = val;
		}
	}
</script>
...
<script lang="ts">
	...
	const updateArmMaterial = (arm: Scene | null, currentArm: Scene | null) => {
		if (arm && currentArm === arm) {
			const color = new Color(material.color as string);

			arm.traverse((node) => {
				if (node instanceof Mesh && node.material && !Array.isArray(node.material)) {
	...
	onMount(() => {
		...
		(async () => {
			...
			arm1 = a1.scene.children[0] as Scene;
			arm2 = a2.scene.children[0] as Scene;
